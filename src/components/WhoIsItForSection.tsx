    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-6 sm:mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-5">
          {whoIsItForCards.map((card, idx) => (
            ...
            >
              ...
              <p className="mt-2.5 sm:mt-3 text-slate-600 font-normal text-xs sm:text-sm leading-relaxed">
                {card.desc}
              </p>
            </div>
          ))}
        ...
        <div className="mt-8 sm:mt-12 max-w-2xl mx-auto flex flex-col items-center">
          <div className="z-10 -mb-3.5 inline-flex items-center gap-1.5 px-4 sm:px-5 py-1 sm:py-1.5 rounded-full bg-red-600 text-white font-black text-[11px] sm:text-sm uppercase tracking-wider shadow-md border-2 border-white">
            <span>⚠️ ATENÇÃO</span>
          </div>
          ...
          <div className="w-full rounded-2xl bg-white border-2 border-red-500 pt-6 sm:pt-7 pb-4 sm:pb-5 px-3.5 sm:px-6 shadow-sm relative overflow-hidden">
            <div className="text-center mb-3">
              <h3 className="font-black text-red-700 text-xs sm:text-base tracking-wide uppercase">
                NÃO É PARA VOCÊ SE...
              </h3>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5">
              {notForYouPoints.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-2 p-2 sm:p-2.5 rounded-xl bg-red-50/80 border border-red-100 text-slate-700 text-[11.5px] sm:text-[13px] font-medium leading-snug"
                >
